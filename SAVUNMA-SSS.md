...
## 15. flq.social'a (veya bir yazıya) erişim engeli gelirse, kullanıcı yurtdışından verisine nasıl erişir?

**Temel ilke:** Erişim engeli **ağ katmanındadır** (Türkiye DNS/IP/SNI filtresi), **veri katmanında değil**. Sunucu (Hetzner, **Almanya**) ve verin olduğu gibi durur; engel sadece o ağdan **giden kapıyı** kapatır. Çözüm "veriyi kurtarmak" değil, **kapıya başka yoldan ulaşmak**tır.

**Tek bir yazı engellenirse:** İçerik sunucuda durur; VPN/Tor/alternatif DNS ile erişilir, ayrıca yazının **federe kopyası** takipçilerin yurtdışı sunucularında okunabilir.

**Tüm siteye erişim engeli gelirse** (en hızlıdan kalıcıya):
1. **VPN / Tor / alternatif DNS** — flq.social ve `api.flq.social`'a doğrudan bağlanır, hesabına girersin. Sunucu yurtdışında olduğu için engel yalnız ağ-seviyesi bir perdedir.
2. **Veri dışa aktarma (export)** — VPN ile bir kez girip JSON arşivini indir; verin artık **cihazında**, Türkiye ağına bağımlı değil.
3. **Hesap taşıma (`Move`)** — hesabını yurtdışındaki başka bir sunucuya taşı; **takipçilerin seninle gelir**, sonraki engeller seni etkilemez.
4. **Self-host** — kendi sunucunu başka ülkede kurar, ağa eşit katılırsın.
5. **Sunucudan bağımsız kimlik** — DID:key + Nostr anahtarların sende; kimliğin tek bir sunucuya/ülkeye bağlı değil.

**Dürüst uyarı:** Export/Move yapabilmek için sunucuya **en az bir kez** (VPN/Tor ile) ulaşman gerekir. Hiç export etmediysen ve hiç bağlanamıyorsan o an canlı veriye ulaşamazsın — ama bu **pratikte aşılabilir** bir engeldir, kalıcı kayıp değil. Doğru alışkanlık: veriyi önceden export et / kritik anda hesabı taşı.

> **15 sn:** Engel ağ katmanında, veri Almanya'daki sunucuda sağlam. VPN/Tor ile doğrudan erişir; verini indirir; hesabını takipçileriyle başka ülkeye taşır; istersen self-host eder. Merkezi platformda "yasak = bitti"; federe ağda yasak tek bir ağ kapısını kapatır, ağı veya kimliğini değil.

---

## 16. Sunucular birbiriyle nasıl konuşur? (Federasyon protokolü)

floq, fediverse'in ortak dili **ActivityPub** (W3C standardı) ile konuşur. İki sunucu arasında bir gönderinin teslimi şu üç adımda olur:

### a) WebFinger → kullanıcıyı bul
Adres bir e-posta gibidir: `@ayse@flq.social`. Uzak sunucu önce **"bu kullanıcı kim, profili nerede?"** diye sorar. Bunu **WebFinger** ile yapar: `GET https://flq.social/.well-known/webfinger?resource=acct:ayse@flq.social`. Cevap, kullanıcının **actor (profil) adresini** döner. Yani WebFinger = ağdaki **telefon rehberi/adres çözümleme**.

### b) HTTP Signatures → kimliği doğrula
İnternet açık olduğundan, "ben falanca sunucuyum" diyen herkese güvenilemez. Bu yüzden her sunucular-arası istek, gönderen sunucunun **özel anahtarıyla imzalanır** (`Signature` başlığı). Alıcı, gönderenin **public anahtarıyla** imzayı doğrular — sahte/taklit sunucu (spoofing) engellenir. floq bunu RSA-SHA256 ile, Node'un yerleşik kripto modülüyle **elle** yapar ([httpSignatures.ts](apps/api/src/lib/httpSignatures.ts)). Yani HTTP Signatures = mektubun üstündeki **mühür/ıslak imza**: kimden geldiği kanıtlanır.

### c) inbox / outbox → gönderiyi teslim et
Her kullanıcının iki posta kutusu vardır:
- **outbox** — kullanıcının ürettiği gönderiler (giden kutusu).
- **inbox** — başka sunuculardan gelen aktiviteler (gelen kutusu).

Ayşe bir gönderi paylaşınca, floq bunu bir **Activity** (`Create`, `Follow`, `Like`, `Announce`, `Delete`…) olarak paketler ve takipçilerin **uzak sunucudaki inbox'larına** imzalı bir `POST` ile teslim eder ([activitypub.ts](apps/api/src/routes/activitypub.ts)). Gelen aktiviteler de kendi inbox'umuza düşer, imzası doğrulanır ve işlenir. Yani inbox/outbox = sunucular arası **posta dağıtım sistemi**.

### d) "Hazır kütüphane değil — sıfırdan yazıldı"
floq, hazır bir ActivityPub kütüphanesi (Fedify, activitypub-express vb.) **kullanmaz**; WebFinger, HTTP Signatures imzalama/doğrulama, actor belgeleri ve inbox/outbox işleme **sıfırdan, protokol spesifikasyonuna göre** yazılmıştır (`package.json`'da fediverse bağımlılığı yoktur). Bu, projenin **akademik/teknik katkısıdır**: protokolü kullanmak değil, **protokolü implemente etmek** — federasyonun nasıl çalıştığını satır satır gösteren bir referans implementasyon. Üç farklı protokolü (ActivityPub + AT Protocol + Nostr) tek kimlik altında köprülemek de bunu mümkün kıldı.

> **15 sn (sunum anlatımı):** İki sunucu üç adımda konuşur: önce **WebFinger** ile kullanıcıyı bulur (adres rehberi), sonra **HTTP Signatures** ile gönderenin kimliğini imzayla doğrular (sahte sunucuyu engeller), sonra gönderiyi alıcının **inbox**'ına imzalı teslim eder (outbox'tan inbox'a posta sistemi). Ve bunların hiçbiri hazır kütüphane değil — WebFinger'dan imzaya kadar protokolü **sıfırdan** yazdım; projenin teknik katkısı da bu.
