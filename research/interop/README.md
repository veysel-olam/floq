...
```bash
# Katman 1 — ÇIKIŞ (floq → hedef) statik matrisi:
npx tsx research/interop/audit.ts            # → MATRIX.md + matrix.csv

# Katman 1 — GİRİŞ (hedef → floq) statik matrisi + yön asimetrisi (RQ1):
npx tsx research/interop/audit-inbound.ts    # → MATRIX_INBOUND.md + matrix-inbound.csv

# Katman 2 — ampirik round-trip (gerçek ağlardan geri çekip doğrula):
#   .env (aşağıdaki değişkenler) gerekir, sonra:
npx tsx research/interop/harness.ts          # → data/empirical-<tarih>.json
```

### Ampirik harness yapılandırması (`.env`)
Adaptörler SDK'sız: AP imzasız public GET, Bluesky public XRPC AppView, Nostr ham relay WebSocket.

| Değişken | Açıklama |
|---|---|
| `FLOQ_API_URL` | örn. `https://api.flq.social` |
| `FLOQ_COOKIE` | Better-Auth oturum çerezi (tarayıcı devtools'tan kopyala) |
| `BLUESKY_HANDLE` | bağlı Bluesky handle (public feed okunur) |
| `NOSTR_NPUB_HEX` | floq Nostr kimliğinin hex pubkey'i |
| `NOSTR_RELAYS` | virgüllü liste (varsayılan: damus + nos.lol) |

> Katman 2, Katman 1'i doğrular **ve** alıcının kurtardığı durumları yakalar
> (ör. Mastodon, metindeki `@`'dan mention'ı çıkarsayabilir → statik "0" → ampirik "0.5").
